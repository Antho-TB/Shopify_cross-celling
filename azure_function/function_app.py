...
def daily_cross_sell_scanner(myTimer: func.TimerRequest) -> None:
    ...
    delay_days = int(os.environ.get("ORDER_DELAY_DAYS", 180))
    ...
    # 2. Trouver les clients ayant commandé il y a X jours
    customers = helper.get_eligible_customers(days_ago=delay_days)
    logging.info(f"Nombre de clients potentiels (J-{delay_days}): {len(customers)}")
